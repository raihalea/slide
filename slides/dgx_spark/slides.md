...
| 使わなかった機能 | 理由 |
|---|---|
| **WAF**（GitHub の送信元 IP に限定） | GitHub の IP レンジは**変動する**ので、許可リストの更新もれが怖い。webhook は署名検証（HMAC）が主防御で、こちらは IP が変わっても影響を受けない |
| **DLP**（流出検知） | **本命だった**が、無料プランでは欲しい機能（秘密鍵や API キーの検出）が使えない |
| **AI Gateway**（レビュー経路に挟む） | レビューのような長い応答は **125秒でタイムアウト**してしまう（エラー 524）。プロンプトが毎回 Cloudflare を経由するのも、ローカルLLMの意義を削る |
